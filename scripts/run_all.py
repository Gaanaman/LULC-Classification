...
def main():
    ...
    # 1. Download EuroSAT
    run_command("python -m src.data.download_eurosat", "Downloading EuroSAT Dataset")
    
    for config in configs:
        model_name = Path(config).stem
        
        # 2. Train
        run_command(f"python -m src.training.train --config {config}", f"Training {model_name}")
        
        # 3. Evaluate
        checkpoint = f"outputs/checkpoints/{model_name}/best_model.pth"
        run_command(f"python -m src.eval.evaluate --config {config} --checkpoint {checkpoint}", f"Evaluating {model_name}")
        ...
        run_command(f"python -m src.visualization.plots --history {history} --cm {cm} --out {out_dir}", f"Plotting metrics for {model_name}")
